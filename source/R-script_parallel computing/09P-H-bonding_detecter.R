...
#==========
case.name  <-  np.case[as.integer(val[1])]
print(case.name)
  ...
  for (j in 1:nrow(target.ca)) {
    ...
    for (k in 1:(length(id.wt)/2)) {
      if ((length(id.wt)/2) > 1) {
        ...
        detC     <-  as.numeric(names(dis.wt[id.wt[1], order(dis.wt[id.wt[1], ])][1:3]))
      }
    ...
    for (k in 1:(length(id.mut)/2)) {
      if ((length(id.mut)/2) > 1) {
        detC     <-  as.numeric(names(dis.mut[id.mut[k, 1], order(dis.mut[id.mut[k, 1], ])][1:3]))
      } else {
        detC     <-  as.numeric(names(dis.mut[id.mut[1], order(dis.mut[id.mut[1], ])][1:3]))
      }
